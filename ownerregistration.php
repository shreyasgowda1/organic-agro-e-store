  ...
  error_reporting(0);
    ...
    if (isset($_REQUEST['username'])){
		...
		$trn_date = date("Y-m-d H:i:s");
		 ...
		 $query = mysqli_query($con, "SELECT * FROM owner WHERE username = '".$username. "'");
  if(mysqli_num_rows(  $query) > 0){
   echo "<div class='form'><h3>!!!!!!!!!!! user name already exist please user other.</h3><br/>Click here to try again <a href='ownerregistration.php'>Register</a></div>";
}
else{
 $query = "INSERT into `owner` (username, password, email) VALUES ('$username', '".md5($password)."', '$email')";
   ...
   ini_set("SMTP", "smtpout.secureserver.net");//confirm smtp
      ...
      mail($to,$subject,$message,$headers);
		...
		exit();
            ...
            echo "<div class='form'><h3>You are registered successfully.</h3><br/>Click here to <a href='ownerlogin.php'>Login</a></div>";
        }
    }else{
?>
    <!-- Navbar-->
    <header class="app-header"><a class="app-header__logo" href="http://localhost/pocketclinic/">Organic Store </a>
...
<p>Already registered ? <a href='ownerlogin.php'>Login Here</a></p></div>
	<?php } ?>
          </div>
        </div>
		</div>
